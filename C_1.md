...
$$z_t = \sigma(W_z \cdot [h_{t-1}, x_t] + b_z)$$ $$r_t = \sigma(W_r \cdot [h_{t-1}, x_t] + b_r)$$
$$\tilde{h}_t = \tanh(W \cdot [r_t * h_{t-1}, x_t] + b)$$ $$h_t = (1 - z_t) * h_{t-1} + z_t * \tilde{h}_t$$
...
- **Recurrent Networks**
    ...
    - More complex gradient computation:
        $$
        \frac{\partial L}{\partial W} = \sum_{t=1}^{T} \frac{\partial L}{\partial y_t} \frac{\partial y_t}{\partial h_t}
...
- Input state: $$h_t = \tanh(W_x \cdot x_t + W_h \cdot h_{t-1} + b_h)$$
...
3. **Weight Updates** $$W_{xh_{new}} = W_{xh} - learning\_rate * \frac{\partial E}{\partial W_{xh}}$$
   $$W_{hh_{new}} = W_{hh} - learning\_rate * \frac{\partial E}{\partial W_{hh}}$$
   $$W_{hy_{new}} = W_{hy} - learning\_rate * \frac{\partial E}{\partial W_{hy}}$$
